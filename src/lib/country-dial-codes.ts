/**
 * Country code (ISO 3166-1 alpha-2), name, and E.164 dial code for phone input.
...
/** Find country by dial code; if multiple (e.g. 1 for US/CA), prefer countryHint. */
export function findCountryByDial(
  dial: string,
  countryHint?: string | null
): CountryDial | undefined {
  const withDial = COUNTRY_DIAL_LIST.filter((c) => c.dial === dial);
  if (withDial.length === 0) return undefined;
  if (countryHint && withDial.some((c) => c.code === countryHint))
    return withDial.find((c) => c.code === countryHint);
  return withDial[0];
}

/** Parse E.164 or plain number into country and national number. Dial codes tried longest first. */
export function parsePhoneForDisplay(
  phone: string | null | undefined,
  countryHint?: string | null
): { country: CountryDial; nationalNumber: string } | null {
  if (!phone || !phone.trim()) return null;
  const digits = phone.replace(/\D/g, "");
  if (digits.length === 0) return null;
  const sorted = [...COUNTRY_DIAL_LIST].sort(
  ...
  for (const country of sorted) {
    if (digits.startsWith(country.dial)) {
      ...
      const resolved =
        country.dial === "1" || country.dial === "7"
          ? findCountryByDial(country.dial, countryHint) ?? country
          : country;
      return { country: resolved, nationalNumber: national };
    }
  }
  return null;
}
...
export const MAX_PHONE_DIGITS = 18;

export function validatePhoneNational(nationalNumber: string): string | null {
  const digits = nationalNumber.replace(/\D/g, "");
  if (digits.length === 0) return null;
  if (digits.length < MIN_PHONE_DIGITS)
    return `Phone number must be at least ${MIN_PHONE_DIGITS} digits`;
  if (digits.length > MAX_PHONE_DIGITS)
    return `Phone number must be at most ${MAX_PHONE_DIGITS} digits`;
  return null;
}
