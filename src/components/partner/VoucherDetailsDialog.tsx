    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Complete information for voucher {voucher.voucher_code}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center p-3 bg-muted/30 rounded-lg">
                <span className="text-sm text-muted-foreground">Voucher Amount</span>
              ...
              {voucher.type === 'sent' && (
                <>
                  <div className="flex justify-between items-center p-3 bg-red-500/10 rounded-lg">
                    <span className="text-sm text-muted-foreground">Partner Paid</span>
            ...
            <div className="space-y-3">
              <div className="p-3 bg-muted/30 rounded-lg">
                <p className="text-sm text-muted-foreground mb-1">
                  {voucher.type === 'sent' ? 'Created By' : 'Partner'}
                </p>
              ...
              {voucher.redeemed_by_user_id && (
                <div className="p-3 bg-muted/30 rounded-lg">
                  <p className="text-sm text-muted-foreground mb-1">Redeemed By</p>
                  <p className="font-medium">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold">Transaction References</h3>
            </div>
            <div className="space-y-2">
              {voucher.purchase_transaction_id && (
                <div className="p-3 bg-muted/30 rounded-lg">
                  <p className="text-xs text-muted-foreground mb-1">Purchase Transaction ID</p>
                  <p className="font-mono text-sm break-all">{voucher.purchase_transaction_id}</p>
                </div>
              )}
              {voucher.redemption_transaction_id && (
                <div className="p-3 bg-muted/30 rounded-lg">
                  <p className="text-xs text-muted-foreground mb-1">Redemption Transaction ID</p>
              ...
              {!voucher.purchase_transaction_id && !voucher.redemption_transaction_id && (
                <p className="text-sm text-muted-foreground italic">No transaction IDs available</p>
              )}
            </div>
          </div>
          ...
          <div className="pt-2 border-t">
            <p className="text-xs text-muted-foreground">
              Voucher ID: <span className="font-mono">{voucher.id}</span>
            </p>
          </div>
        </div>
      </DialogContent>
