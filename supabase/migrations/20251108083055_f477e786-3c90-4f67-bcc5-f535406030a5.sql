-- Add metadata column to withdrawal_requests table
ALTER TABLE withdrawal_requests 
ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT '{}'::jsonb;

-- Add index for faster metadata queries
CREATE INDEX IF NOT EXISTS idx_withdrawal_requests_metadata ON withdrawal_requests USING GIN (metadata);

-- Add comment for documentation
COMMENT ON COLUMN withdrawal_requests.metadata IS 'Stores additional withdrawal details like cryptocurrency type, network, etc.';

-- Update process_withdrawal_request_atomic function to accept and store metadata
CREATE OR REPLACE FUNCTION public.process_withdrawal_request_atomic(
  ...
  p_payment_processor_id uuid DEFAULT NULL::uuid,
...
BEGIN
  ...
  -- Step 5: Create withdrawal request record with metadata
  ...
  -- Step 6: Create transaction record with metadata
