-- =============================================================================
-- Run this in Supabase Dashboard → SQL Editor (one block)
-- Fix: New users / NULL membership_plan → "unknown" plan, 0 tasks, no task access
-- =============================================================================

-- 1) Backfill: fix existing users with no plan
UPDATE public.profiles
SET membership_plan = 'free'
WHERE membership_plan IS NULL OR TRIM(membership_plan) = '';

-- 2) Ensure column default
ALTER TABLE public.profiles
  ALTER COLUMN membership_plan SET DEFAULT 'free';

-- 3) handle_new_user: set membership_plan = 'free' for new signups
...
-- 4) get_user_detail_aggregated: resolve NULL/empty membership_plan as 'free' for plan_info
