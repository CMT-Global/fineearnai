-- Fix signup with referral code: "Database error saving new user"
-- Migration 20260217000000 replaced handle_new_user with a version that inserts into
-- referrals(referred_user_id, signup_date), but the referrals table has referred_id
-- and referral_code_used. That INSERT fails when a referral code is present.
-- This migration restores the correct handle_new_user: use referred_id and
-- referral_code_used, and restore membership_plan + user_roles.
...
COMMENT ON FUNCTION public.handle_new_user() IS 'Creates profile and user_roles on signup; uses referrals(referred_id, referral_code_used). Fixes signup with referral code.';
