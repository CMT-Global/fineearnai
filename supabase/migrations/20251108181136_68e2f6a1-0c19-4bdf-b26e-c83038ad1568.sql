-- CORRECTIVE MIGRATION: Fix Phase 1 bugs in credit_deposit_atomic_v2
-- Fix 1: Change new_balance_after_transaction to new_balance
-- Fix 2: Change deposit_commission to referral_commission
...
BEGIN
  ...
  -- Create deposit transaction (FIX #1: Changed to new_balance)
  ...
  -- ========================================
  -- SIMPLIFIED COMMISSION PROCESSING (NO PLAN ELIGIBILITY CHECKS)
  -- ========================================
  ...
  IF v_referral IS NOT NULL THEN
    ...
    -- Get upline's deposit commission rate (NO account_type check)
    SELECT 
      mp.name, 
    ...
    ELSE
      ...
      -- Create commission transaction for upline (FIX #1 & #2: Changed to new_balance and referral_commission)
      INSERT INTO transactions (
        user_id,
        type,
        amount,
