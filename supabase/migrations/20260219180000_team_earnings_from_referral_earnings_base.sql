-- ============================================
-- Team earnings = sum of base_amount from referral_earnings (task_commission) in period
-- Only counts referral task earnings that generated a commission for the referrer.
-- ============================================
...
BEGIN
  ...
  top AS (
    SELECT
      ...
      SUM(pe.base_amount)::NUMERIC AS their_earnings,
      COALESCE(SUM(pe.commission_amount), 0)::NUMERIC AS your_commission
    FROM period_earnings pe
    LEFT JOIN public.profiles p ON p.id = pe.referred_user_id
    GROUP BY pe.referred_user_id, p.username
...
'Returns period-based referral analytics. team_earnings_sum = sum(base_amount) from referral_earnings (task_commission) in period.';
