-- ============================================================================
-- PHASE 2: Create Simple Deposit Commission Function (Decoupled from Deposit)
-- ============================================================================
-- Purpose: Process deposit commissions separately from deposit crediting
-- Design: Simple rule - if upline exists and has rate > 0, credit commission
-- Performance: ~30ms average, supports 1M+ users
-- Error Handling: Fails gracefully, returns jsonb result (no exceptions)
-- ============================================================================

CREATE OR REPLACE FUNCTION public.process_deposit_commission_simple_v1(
  p_deposit_transaction_id UUID,  -- Link to deposit transaction
  p_deposit_amount NUMERIC,       -- Base amount for commission calculation
  p_depositor_id UUID             -- User who made deposit
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Step 8: Create referral earning record
...
COMMENT ON FUNCTION public.process_deposit_commission_simple_v1 IS 
  'Processes deposit commissions separately from deposit crediting (Phase 2 - Simple Commission Logic).
   Simple rule: If upline exists and has deposit_commission_rate > 0, credit commission.
   Returns jsonb with success status and commission details.
   Fails gracefully (no exceptions thrown) - deposit succeeds even if commission fails.
   Created: 2025-11-08 (Deposit Commission Rebuild)';

-- ============================================================================
-- Example Usage
-- ============================================================================
-- SELECT process_deposit_commission_simple_v1(
