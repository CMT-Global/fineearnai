-- Fix purchase_voucher_atomic: Insert transaction BEFORE updating profile
-- This aligns with validate_transaction_balance which expects to read the OLD balance
...
BEGIN
  ...
  -- Step 4: Calculate new balance with explicit rounding
  v_new_balance := ROUND(v_current_balance - ROUND(p_partner_paid_amount, 2), 2);
  
  RAISE NOTICE '[VOUCHER-ATOMIC] Partner: %, Amount: %, Old Balance: %, New Balance: %', 
    p_partner_id, ROUND(p_partner_paid_amount, 2), v_current_balance, v_new_balance;
  
  -- Step 5: Create transaction record BEFORE updating profile
  -- This ensures the validator reads the OLD balance from profiles
  INSERT INTO public.transactions (
    ...
    jsonb_build_object(
      ...
      'voucher_amount', ROUND(p_voucher_amount, 2),
      'commission_amount', ROUND(p_commission_amount, 2),
  ...
  RAISE NOTICE '[VOUCHER-ATOMIC] Transaction created: %, new_balance: %', v_transaction_id, v_new_balance;
  
  -- Step 6: Update partner balance AFTER transaction insert
  -- The validator has already validated against the old balance
  ...
  RAISE NOTICE '[VOUCHER-ATOMIC] Profile updated to balance: %', v_new_balance;
  ...
  INSERT INTO public.vouchers (
    ...
    p_voucher_code,
    p_partner_id,
    ROUND(p_voucher_amount, 2),
    ROUND(p_partner_paid_amount, 2),
    ROUND(p_commission_amount, 2),
    p_commission_rate,
    'active',
  ...
  -- Step 8: Update partner stats with rounded amounts
  UPDATE public.partner_config
  SET 
    total_vouchers_sold = total_vouchers_sold + 1,
    total_commission_earned = total_commission_earned + ROUND(p_commission_amount, 2),
    daily_sales = daily_sales + ROUND(p_voucher_amount, 2),
    weekly_sales = weekly_sales + ROUND(p_voucher_amount, 2),
    last_sale_at = NOW(),
    updated_at = NOW()
  WHERE user_id = p_partner_id;
  ...
  RETURN jsonb_build_object(
    ...
    'amount_charged', ROUND(p_partner_paid_amount, 2),
    'commission_earned', ROUND(p_commission_amount, 2),
    'expires_at', v_voucher_expires_at,
    'new_rank', v_new_rank,
    'total_vouchers_sold', v_partner_config.total_vouchers_sold + 1,
    'daily_sales', v_partner_config.daily_sales + ROUND(p_voucher_amount, 2)
  );
