-- Fix: Admin influencer summary must show the same commission totals as the affiliate's "Your Team" page.
-- The referral page uses SUM(referrals.total_commission_earned). Use that as fallback when referral_earnings
-- or transactions undercount (e.g. timing or source mismatch).
...
BEGIN
    ...
    SELECT
      ...
      COALESCE(
        NULLIF((
          ...
          WHERE re.referrer_id = ap.pid AND re.earning_type = 'deposit_commission'
          AND (p_date_from IS NULL OR re.created_at >= p_date_from)
          AND (p_date_to IS NULL OR re.created_at <= p_date_to)
        ), 0),
        (SELECT COALESCE(SUM(t.amount), 0) FROM transactions t
          WHERE t.user_id = ap.pid AND t.type = 'referral_commission'
      ...
      ) AS deposit_from_earnings,
      COALESCE(
        NULLIF((
          SELECT SUM(re.commission_amount) FROM referral_earnings re
          WHERE re.referrer_id = ap.pid AND re.earning_type = 'task_commission'
          AND (p_date_from IS NULL OR re.created_at >= p_date_from)
          AND (p_date_to IS NULL OR re.created_at <= p_date_to)
        ), 0),
        (SELECT COALESCE(SUM(t.amount), 0) FROM transactions t
          ...
          AND (p_date_from IS NULL OR t.created_at >= p_date_from)
          AND (p_date_to IS NULL OR t.created_at <= p_date_to))
      ) AS task_from_earnings,
      CASE WHEN p_date_from IS NULL AND p_date_to IS NULL THEN
        (SELECT COALESCE(SUM(r.total_commission_earned), 0) FROM referrals r WHERE r.referrer_id = ap.pid)
      ELSE 0 END AS total_from_referrals_table
    FROM affiliate_profiles ap
  ),
  adjusted AS (
    SELECT
      cs.pid,
      cs.deposit_from_earnings AS total_deposit_commissions,
      -- Ensure deposit + task >= total_from_referrals (match affiliate page when no date filter)
      cs.task_from_earnings + GREATEST(0,
        cs.total_from_referrals_table - (cs.deposit_from_earnings + cs.task_from_earnings)
  ...
  SELECT
    ...
    COALESCE(adj.total_deposit_commissions, 0)::numeric,
    COALESCE(adj.total_task_commissions, 0)::numeric,
    (SELECT COALESCE(SUM(t.amount), 0) FROM transactions t
  ...
  FROM profiles p
  INNER JOIN user_affiliate_settings uas ON uas.user_id = p.id
  LEFT JOIN adjusted adj ON adj.pid = p.id
  WHERE uas.is_affiliate = true
  ORDER BY (
...
COMMENT ON FUNCTION public.get_influencer_summary(timestamptz, timestamptz) IS
  'Returns one row per influencer (is_affiliate=true). Commission totals match affiliate Your Team page: uses referral_earnings/transactions and falls back to SUM(referrals.total_commission_earned) when no date filter.';
