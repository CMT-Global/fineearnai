-- Fix: Ensure get_user_detail_aggregated function exists with all fields
-- This migration ensures the function is created/updated with all latest fields including IPStack location data
  ...
  SELECT json_build_object(
    ...
    'financial', json_build_object(
      ...
      'lifetime_net_earnings', (
        COALESCE(p.total_earned, 0) + COALESCE((
          SELECT SUM(amount) 
          FROM transactions 
          WHERE user_id = p_user_id 
            AND type = 'deposit' 
    ...
    -- Upline/referrer information (Uses referrals table as source of truth)
    'upline', (
      SELECT json_build_object(
        'id', up.id,
    ...
    -- Referral relationship details (Uses referrals table as source of truth)
    'referral_details', (
      SELECT json_build_object(
        'referral_id', r.id,
        'status', r.status,
...
Includes IPStack location data (registration and last login IP/country).
