-- Phase 3: Update Database Function - Change payment_processor_id parameter to UUID

-- Update process_withdrawal_request_atomic function to use UUID parameter
CREATE OR REPLACE FUNCTION public.process_withdrawal_request_atomic(
  ...
  p_payment_processor_id uuid DEFAULT NULL  -- Changed from TEXT to UUID
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO withdrawal_requests (
    ...
    p_payout_address,
    p_payment_method,
    p_payment_processor_id,  -- Now properly UUID type
    'pending',
    NOW()
  )
