-- Fix influencer summary: referred_free/referred_upgraded use LEFT JOIN to membership_plans so referred users with missing/unknown plan are counted (treat as free). Only show users who are affiliates.
...
BEGIN
  ...
  SELECT
    ...
    -- Count referred users on free vs upgraded: match plan case-insensitively so e.g. Beginner is correctly counted as upgraded
    ...
    -- Deposit commissions: prefer referral_earnings; fallback to transactions (referral_commission from deposit)
    COALESCE(
      NULLIF((
    ...
    -- Task commissions: prefer referral_earnings; fallback to transactions (referral_commission from task_completion) so admin summary matches affiliate dashboard
    COALESCE(
      NULLIF((
        SELECT SUM(re.commission_amount) FROM referral_earnings re
...
COMMENT ON FUNCTION public.get_influencer_summary(timestamptz, timestamptz) IS
  'Returns one row per influencer (is_affiliate=true): referred counts (free/upgraded), deposits by referred users, commission totals, withdrawn, current balance. Optional date filter.';
