-- Create ai_tasks table (idempotent)
...
-- Create task_completions table (idempotent)
...
-- RLS Policies for ai_tasks (idempotent)
DO $$ 
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'ai_tasks' 
    AND policyname = 'Anyone can view active AI tasks'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'ai_tasks' 
    AND policyname = 'Admins can manage AI tasks'
  ) THEN
...
-- RLS Policies for task_completions (idempotent)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'task_completions' 
    AND policyname = 'Admins can view all completions'
  ) THEN
...
CREATE INDEX IF NOT EXISTS idx_ai_tasks_difficulty ON public.ai_tasks(difficulty);
