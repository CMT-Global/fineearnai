...
DECLARE
  ...
  v_referred_plan_eligible BOOLEAN; -- NEW: Check if referred user's plan is eligible
...
BEGIN
  ...
  -- Step 10: COMMISSION PROCESSING with referral eligibility check
  IF p_earnings_amount > 0 THEN
    RAISE NOTICE '[COMMISSION] Starting task commission check for user=%', p_user_id;
    
    -- PHASE 2: Check if referred user's plan is referral eligible
    ...
    -- Skip commission if referred user's plan is not eligible
    IF v_referred_plan_eligible IS NULL OR v_referred_plan_eligible = false THEN
      ...
      -- Early return, no commission processing
    ELSE
      -- Get active referral
      ...
      IF v_referral.id IS NOT NULL THEN
        ...
        IF v_referrer_plan.id IS NOT NULL THEN
          ...
          RAISE NOTICE '[COMMISSION] Referrer plan: %, task_commission_rate: %', v_referrer_plan.name, v_commission_rate;
          ...
          IF v_commission_rate > 0 THEN
            ...
            RAISE NOTICE '[COMMISSION] SUCCESS: Task commission processed: referrer=%, amount=%', v_referral.referrer_id, v_commission_amount;
          ELSE
            RAISE NOTICE '[COMMISSION] SKIPPED: Referrer commission rate is zero';
          END IF;
        ELSE
...
BEGIN
  ...
  -- Step 6: COMMISSION PROCESSING with referral eligibility check
  ...
  -- Skip commission if referred user's plan is not eligible
  IF v_referred_plan_eligible IS NULL OR v_referred_plan_eligible = false THEN
    RAISE NOTICE '[COMMISSION] SKIPPED: Referred user is on non-eligible plan (referral_eligible=%)', v_referred_plan_eligible;
    -- Early return, no commission processing
  ELSE
    -- Get active referral
