...
BEGIN
  -- Check if referred_by column exists in profiles table
  SELECT EXISTS (
    SELECT 1
  ...
  IF column_exists THEN
    ...
    -- Step 3: Count users with referred_by
    SELECT COUNT(*) INTO total_referred_by
    FROM public.profiles
    WHERE referred_by IS NOT NULL;
    
    RAISE NOTICE 'Phase 1 Complete: Backfilled % referrals. Users with referred_by: %', 
      backfill_count, total_referred_by;
  ELSE
    RAISE NOTICE 'Phase 1: referred_by column does not exist in profiles table. Skipping backfill.';
  END IF;
  ...
  RAISE NOTICE 'Total referrals records: %', total_referrals;
END $$;