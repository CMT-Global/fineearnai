-- Use influencer/affiliate deposit_commission_pct when referrer has affiliate settings.
-- Affiliate rate is stored as 0-100 (e.g. 7.5); convert to decimal for calculation.
...
BEGIN
  ...
  -- Step 3: Prefer affiliate rate when referrer is influencer with deposit_commission_pct set
  SELECT uas.deposit_commission_pct INTO v_affiliate_rate
  FROM user_affiliate_settings uas
  WHERE uas.user_id = v_referral.referrer_id
    AND uas.is_affiliate = true
    AND uas.deposit_commission_pct IS NOT NULL
  LIMIT 1;

  IF v_affiliate_rate IS NOT NULL THEN
    v_commission_rate := v_affiliate_rate / 100.0;
    v_rate_source := 'affiliate';
    SELECT username INTO v_upline_username FROM profiles WHERE id = v_referral.referrer_id;
    RAISE WARNING '[COMMISSION] Step 3: Using affiliate rate=% (source=affiliate)', v_commission_rate;
  END IF;

  -- Step 4: If no affiliate rate, get upline plan and use plan rate
  IF v_rate_source = 'plan' THEN
    SELECT
      mp.name,
  ...
  -- Step 7: Lock upline profile and credit commission
  ...
  -- Step 8: Create commission transaction (include rate_source and commission_rate_used for audit)
  ...
  -- Step 9: Create referral earning record
  ...
  RAISE WARNING '[COMMISSION] SUCCESS: Processed commission=% for upline=%',
    v_commission_amount, v_upline_username;
