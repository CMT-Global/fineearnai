-- Fix Top Referrers deposits to only count payment processor transactions
-- Excludes admin adjustments and manual entries (only deposits with payment_gateway)
...
BEGIN
  ...
  WITH referrer_stats AS (
    SELECT 
      ...
      COALESCE(SUM(
        CASE 
          WHEN t.type = 'deposit' AND t.status = 'completed' AND t.payment_gateway IS NOT NULL
          THEN t.amount 
          ELSE 0 
        END
