-- Remove dependency on plan name 'free': backfill profiles from purchases / default tier, fix plan lookup, set column default.
-- Your default (free-tier) plan is identified by account_type = 'free' in membership_plans (e.g. Trainee). No plan named 'free' required.

-- Step 1: Users who have a completed plan_upgrade -> set membership_plan to their latest plan name from transactions.
...
WHERE p.id = lu.user_id
  ...
  AND EXISTS (
    ...
    WHERE mp.name = lu.plan_name AND mp.is_active = true
    LIMIT 1
  );

-- Step 2: Everyone else (no plan_upgrade or invalid plan name) -> set to the current default-tier plan name (account_type = 'free').
...
OR NOT EXISTS (
  SELECT 1 FROM public.membership_plans mp
  WHERE mp.name = TRIM(COALESCE(p.membership_plan, '')) AND mp.is_active = true
);

-- Step 3: Set column default to the default-tier plan name (so any future insert without membership_plan gets the right value).
DO $$
DECLARE
...
BEGIN
  ...
  WHERE account_type = 'free' AND is_active = true
  LIMIT 1;
  IF default_plan_name IS NOT NULL THEN
    EXECUTE format('ALTER TABLE public.profiles ALTER COLUMN membership_plan SET DEFAULT %L', default_plan_name);
  END IF;
END $$;

-- Step 4: Fix get_user_detail_aggregated: use default plan name from table when membership_plan is null/empty (no hardcoded 'free').
