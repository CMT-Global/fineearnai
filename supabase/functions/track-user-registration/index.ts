 ...
 */ Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabase.from('profiles').select('country').eq('id', userId).single();
    if (profileError) {
    ...
    const { data: userProfile } = await supabase.from('profiles').select('email, username, full_name').eq('id', userId).single();
