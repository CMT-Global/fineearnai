...
(function(CheckpointStage) {
  CheckpointStage["START"] = "START";
  CheckpointStage["CORS_CHECK"] = "CORS_CHECK";
  CheckpointStage["AUTH"] = "AUTH";
  CheckpointStage["ROLE_VERIFICATION"] = "ROLE_VERIFICATION";
  CheckpointStage["REQUEST_VALIDATION"] = "REQUEST_VALIDATION";
  CheckpointStage["PARTNER_CONFIG_FETCH"] = "PARTNER_CONFIG_FETCH";
  CheckpointStage["COMMISSION_RATE_FETCH"] = "COMMISSION_RATE_FETCH";
  CheckpointStage["BALANCE_FETCH"] = "BALANCE_FETCH";
  CheckpointStage["BALANCE_CHECK"] = "BALANCE_CHECK";
  CheckpointStage["VOUCHER_CODE_GENERATION"] = "VOUCHER_CODE_GENERATION";
  CheckpointStage["ATOMIC_TRANSACTION_START"] = "ATOMIC_TRANSACTION_START";
  CheckpointStage["ATOMIC_TRANSACTION_COMPLETE"] = "ATOMIC_TRANSACTION_COMPLETE";
  CheckpointStage["NOTIFICATION"] = "NOTIFICATION";
  CheckpointStage["SUCCESS"] = "SUCCESS";
...
function logCheckpoint(correlationId, stage, data) {
...
function isTransientError(error) {
  ...
  return message.includes('timeout') || message.includes('connection') || message.includes('temporary') || message.includes('lock') || message.includes('deadlock');
}
// Retry logic with exponential backoff
async function retryWithBackoff(correlationId, operationName, operation, maxAttempts = MAX_RETRY_ATTEMPTS) {
  let lastError;
  for(let attempt = 1; attempt <= maxAttempts; attempt++){
    try {
      ...
      if (attempt < maxAttempts && isTransientError(error)) {
        ...
        console.warn(`[${correlationId}] Transient error in ${operationName} (attempt ${attempt}/${maxAttempts}):`, error.message, `Retrying in ${delay}ms...`);
        await new Promise((resolve)=>setTimeout(resolve, delay));
      } else {
        // Non-transient error or out of retries
        console.error(`[${correlationId}] Failed ${operationName} after ${attempt} attempt(s):`, error);
        throw error;
      }
    }
  }
...
async function withTimeout(correlationId, operationName, operation, timeoutMs = EDGE_FUNCTION_TIMEOUT_MS) {
...
async function logMetric(supabaseClient, correlationId, userId, success, executionTimeMs, metadata, errorMessage) {
  try {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: { user }, error: authError } = await retryWithBackoff(correlationId, 'authentication', ()=>supabaseClient.auth.getUser());
    if (authError || !user) {
      ...
      await logMetric(supabaseClient, correlationId, null, false, Date.now() - startTime, {
        ...metadata,
        checkpoint: CheckpointStage.AUTH
    ...
    const { data: partnerRole } = await retryWithBackoff(correlationId, 'role_verification', ()=>supabaseClient.from("user_roles").select("role").eq("user_id", user.id).eq("role", "partner").single());
    if (!partnerRole) {
      ...
      await logMetric(supabaseClient, correlationId, userId, false, Date.now() - startTime, {
        ...metadata,
        checkpoint: CheckpointStage.ROLE_VERIFICATION
      }, "User is not a partner");
      return new Response(JSON.stringify({
        error: "Only partners can purchase vouchers"
      }), {
        status: 403,
        headers: {
          ...corsHeaders,
    ...
    if (!body.voucher_amount || body.voucher_amount <= 0) {
      ...
      await logMetric(supabaseClient, correlationId, userId, false, Date.now() - startTime, {
        ...metadata,
        checkpoint: CheckpointStage.REQUEST_VALIDATION,
        invalid_amount: body.voucher_amount
    ...
    const { data: partnerConfig } = await retryWithBackoff(correlationId, 'partner_config_fetch', ()=>supabaseClient.from("partner_config").select("*").eq("user_id", user.id).single());
    if (!partnerConfig) {
      console.error(`[${correlationId}] ❌ Partner config not found for user:`, userId);
      await logMetric(supabaseClient, correlationId, userId, false, Date.now() - startTime, {
        ...metadata,
        error_stage: "partner_config_fetch"
    ...
    if (!partnerConfig.is_active) {
      ...
      await logMetric(supabaseClient, correlationId, userId, false, Date.now() - startTime, {
        ...metadata,
        error_stage: "partner_active_check"
    ...
    const { data: commissionRateData, error: commissionError } = await retryWithBackoff(correlationId, 'commission_rate_fetch', ()=>supabaseClient.rpc("get_partner_commission_rate", {
        p_user_id: user.id
      }));
    if (commissionError) {
      ...
      await logMetric(supabaseClient, correlationId, userId, false, Date.now() - startTime, {
        ...metadata,
        checkpoint: CheckpointStage.COMMISSION_RATE_FETCH
    ...
    const commission_rate = commissionRateData;
    ...
    const { data: profile, error: profileError } = await retryWithBackoff(correlationId, 'balance_fetch', ()=>supabaseClient.from("profiles").select("deposit_wallet_balance, username").eq("id", user.id).single());
    if (profileError) {
      ...
      await logMetric(supabaseClient, correlationId, userId, false, Date.now() - startTime, {
        ...metadata,
        checkpoint: CheckpointStage.BALANCE_FETCH
    ...
    const { data: voucherCode, error: codeError } = await retryWithBackoff(correlationId, 'voucher_code_generation', ()=>supabaseClient.rpc("generate_voucher_code"));
    ...
    if (codeError) {
      ...
      await logMetric(supabaseClient, correlationId, userId, false, Date.now() - startTime, {
        ...metadata,
        checkpoint: CheckpointStage.VOUCHER_CODE_GENERATION,
        code_gen_duration: codeGenDuration
      }, codeError.message);
      throw codeError;
    }
    ...
    const { data: result, error: purchaseError } = await withTimeout(correlationId, 'atomic_transaction', retryWithBackoff(correlationId, 'atomic_transaction', ()=>supabaseClient.rpc("purchase_voucher_atomic", atomicParams)), EDGE_FUNCTION_TIMEOUT_MS);
    ...
    // This would require a new edge function or email template
      ...
      await logMetric(supabaseClient, correlationId, userId, true, Date.now() - startTime, {
        ...metadata,
    ...
    await logMetric(supabaseClient, correlationId, userId, true, totalDuration, metadata);
    logCheckpoint(correlationId, CheckpointStage.SUCCESS, {
    ...
    if (supabaseClient) {
      await logMetric(supabaseClient, correlationId, userId, false, totalDuration, {
        ...metadata,
    ...
    const statusCode = error.message?.includes('timeout') ? 504 : error.message?.includes('Unauthorized') ? 401 : 500;
    return new Response(JSON.stringify({
      error: error.message,
