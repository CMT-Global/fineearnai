...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!bonuses || bonuses.length === 0) {
      ...
      return new Response(
        JSON.stringify({ success: true, message: 'No bonuses to process', bonuses_paid: 0 }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
      );
    }
    ...
    // Process each bonus
    for (const bonus of bonuses as BonusRecord[]) {
      ...
      try {
        ...
        const { data: profile, error: profileError } = await supabase
          .from('profiles')
          .select('earnings_wallet_balance, total_earned, username')
        ...
        if (txError) {
          ...
          await supabase
            .from('profiles')
            .update({ earnings_wallet_balance: oldBalance })
            .eq('id', bonus.partner_id);
