...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!['a', 'b', 'c', 'd'].includes(selectedResponse)) {
      return new Response(JSON.stringify({
        error: 'validation_error',
        message: 'Invalid response selection. Must be "a", "b", "c", or "d"'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    const { data: task, error: taskError } = await supabase.from('ai_tasks_4opt').select('*').eq('id', taskId).eq('is_active', true).maybeSingle();
    if (taskError || !task) {
    ...
    const { data: result, error: atomicError } = await supabase.rpc('complete_task_4opt_atomic', {
      p_user_id: user.id,
      p_task_id: taskId,
    ...
    if (!result || !result.success) {
      ...
      console.error(`❌ [${requestId}] Atomic operation failed (${processingTime}ms):`, {
        errorCode,
        errorMessage
      });
    ...
    if (userId) {
      ...
      const supabase = createClient(supabaseUrl, supabaseServiceKey);
      supabase.from('edge_function_metrics').insert({
        function_name: 'complete-ai-task-4opt',
        execution_time_ms: totalExecutionTime,
        success: false,
