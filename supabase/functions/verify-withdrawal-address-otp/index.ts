...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: otpRecord, error: otpError } = await supabase.from('withdrawal_address_otps').select('*').eq('user_id', user.id).eq('otp_code', otpCode).is('used_at', null).gt('expires_at', new Date().toISOString()).order('created_at', {
      ascending: false
    }).limit(1).maybeSingle();
    if (otpError) {
    ...
    if (otpRecord.attempts >= otpRecord.max_attempts) {
      ...
      await supabase.from('withdrawal_address_otps').update({
