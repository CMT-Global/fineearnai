...
const handler = async (req)=>{
  ...
  try {
    ...
    if (!email || !inviteeName) {
      return new Response(JSON.stringify({
        error: "Email and invitee name are required"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: template, error: templateError } = await supabase.from("email_templates").select("*").eq("template_type", "user_invite").eq("is_active", true).single();
    if (templateError || !template) {
      console.error("[USER-INVITE] Template fetch error:", templateError);
      return new Response(JSON.stringify({
        error: "User invite template not found"
      }), {
        status: 404,
        headers: {
          ...corsHeaders,
    ...
    let personalizedBody = template.body.replace(/{{invitee_name}}/g, inviteeName).replace(/{{platform_url}}/g, platformUrl).replace(/{{signup_bonus}}/g, bonusText).replace(/{{support_email}}/g, emailSettings.reply_to_address);
    ...
    return new Response(JSON.stringify({
      error: error.message || "Failed to send user invite"
    }), {
      status: 500,
      headers: {
        ...corsHeaders,
