...
const handler = async (req)=>{
  ...
  try {
    ...
    const { email, influencerName, commissionRate, customMessage } = await req.json();
    if (!email || !influencerName) {
      return new Response(JSON.stringify({
        error: "Email and influencer name are required"
      }), {
        status: 400,
        headers: {
          "Content-Type": "application/json",
    ...
    const { data: template, error: templateError } = await supabase.from("email_templates").select("*").eq("template_type", "influencer_invite").eq("is_active", true).maybeSingle();
