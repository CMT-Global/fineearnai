...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { action, userIds, planName, suspendReason, exportFormat = 'csv', emailData } = await req.json();
    ...
    switch(action){
      case 'bulk_update_plan':
        {
          ...
          const { data: plan } = await supabaseClient.from('membership_plans').select('*').eq('name', planName).eq('is_active', true).single();
          if (!plan) {
          ...
          for(let i = 0; i < userIds.length; i += batchSize){
            ...
            if (updateError) {
              batch.forEach((id)=>results.failed.push({
                  userId: id,
      ...
      case 'bulk_export':
        {
          // Fetch user data
          const { data: users, error: fetchError } = await supabaseClient.from('mv_user_management').select('*').in('id', userIds);
          if (fetchError) {
          ...
          if (exportFormat === 'csv') {
            ...
            const rows = users.map((user)=>Object.values(user).map((val)=>typeof val === 'string' && val.includes(',') ? `"${val}"` : val).join(','));
            exportData = [
              headers,
              ...rows
            ].join('\n');
      ...
      case 'bulk_email':
        {
          ...
          // Fetch user emails
          const { data: users, error: fetchError } = await supabaseClient.from('profiles').select('id, email, full_name').in('id', userIds);
          if (fetchError) {
          ...
          const { error: insertError } = await supabaseClient.from('email_logs').insert(emailRecords);
          if (insertError) {
