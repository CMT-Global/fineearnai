...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: roleData, error: roleError } = await supabase.from('user_roles').select('role').eq('user_id', user.id).eq('role', 'partner').maybeSingle();
    if (roleError) {
      log({
      ...
      return new Response(JSON.stringify({
        success: false,
        error: 'Failed to check partner role'
      }), {
        status: 500,
        headers: {
          ...corsHeaders,
    ...
    const { data: application, error: appError } = await supabase.from('partner_applications').select('*').eq('user_id', user.id).order('created_at', {
      ascending: false
    }).limit(1).maybeSingle();
    if (appError) {
      log({
