...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    }).or(`last_task_date.is.null,last_task_date.lt.${currentDate}`).select('id, username');
    ...
    const { error: logError } = await supabase.from('daily_reset_logs').insert({
      ...
      details: {
        ...
        user_sample: data?.slice(0, 5).map((u)=>({
            id: u.id,
            username: u.username
          })) || []
      }
