...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!isAdmin) {
      ...
      const { data: recentRequests, error: rateLimitError } = await supabase.from('withdrawal_requests').select('id').eq('user_id', user.id).gte('created_at', oneHourAgo);
    ...
    if (paymentProcessorId) {
      ...
      const { data: processorConfig, error: processorError } = await supabase.from('payment_processors').select('fee_fixed, fee_percentage, name').eq('id', paymentProcessorId).eq('is_active', true).single();
    ...
    const minWithdrawal = parseFloat(plan.min_withdrawal);
    ...
    if (!allowDailyWithdrawals) {
      ...
      if (scheduleError) {
        ...
        return new Response(JSON.stringify({
          error: 'Failed to validate withdrawal schedule'
        }), {
          status: 500,
          headers: {
            ...corsHeaders,
      ...
      if (!isAllowed) {
        ...
        const { data: nextWindowData } = await supabase.rpc('get_next_withdrawal_window').single();
        const nextWindow = nextWindowData;
        ...
        const { data: scheduleConfig } = await supabase.from('platform_config').select('value').eq('key', 'payout_schedule').single();
        ...
        if (scheduleConfig && scheduleConfig.value) {
          ...
          const enabledDays = schedule.filter((s)=>s.enabled).map((s)=>`${dayNames[s.day]} (${s.start_time}-${s.end_time} UTC)`).join(', ');
          if (enabledDays) {
    ...
    const { data: todayWithdrawals } = await supabase.from('withdrawal_requests').select('amount').eq('user_id', user.id).gte('created_at', today).in('status', [
      'pending',
      'approved',
      'processing',
      'completed'
    ]);
    const totalWithdrawnToday = todayWithdrawals?.reduce((sum, t)=>sum + parseFloat(t.amount), 0) || 0;
    const maxDailyWithdrawal = parseFloat(plan.max_daily_withdrawal);
    ...
    const { data: pendingCheck, error: pendingError } = await supabase.from('withdrawal_requests').select('id').eq('user_id', user.id).eq('status', 'pending').maybeSingle();
    if (pendingError) {
    ...
    const { data: result, error: atomicError } = await supabase.rpc('process_withdrawal_request_atomic', {
      p_user_id: user.id,
    ...
    if (!result || !result.success) {
      ...
      console.error('Atomic withdrawal failed:', {
        errorCode,
        errorMessage,
        userId: user.id
      });
