...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!config?.value || config.value === false) {
      return new Response(
        JSON.stringify({ enabled: false, leaderboard: [] }),
    ...
    const leaderboardData = await Promise.all(
      partners.map(async (partner: any) => {
        const { data: salesData } = await supabase
          .from('vouchers')
          .select('voucher_amount, partner_paid_amount, commission_amount')
          .eq('partner_id', partner.user_id)
          .eq('status', 'redeemed')
          .gte('redeemed_at', dateFilter.toISOString());
        ...
        return {
          ...
          total_sales: time_period === 'all_time' ? 
            (await supabase
              .from('vouchers')
              .select('voucher_amount')
              .eq('partner_id', partner.user_id)
              .eq('status', 'redeemed')
              .then(({ data }) => data?.reduce((sum, v) => sum + Number(v.voucher_amount), 0) || 0)
            ) : periodSales,
          total_commission: time_period === 'all_time' ? 
            Number(partner.total_commission_earned) : periodCommission,
          vouchers_sold: time_period === 'all_time' ? 
            Number(partner.total_vouchers_sold) : periodVouchers,
        };
      })
    );
