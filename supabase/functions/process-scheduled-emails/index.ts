...
const handler = async (req)=>{
  ...
  try {
    ...
    const { data: scheduledEmails, error: fetchError } = await supabase.from("scheduled_emails").select("*").eq("status", "pending").lte("scheduled_for", now).order("scheduled_for", {
      ascending: true
    ...
    for (const scheduledEmail of scheduledEmails){
      try {
        ...
        const filter = scheduledEmail.recipient_filter;
        ...
        if (filter.type === "plan" && filter.plan) {
          ...
          const usernameList = filter.usernames.split(",").map((u)=>u.trim());
