...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { partner_id, date_range = 'month' } = await req.json() as AnalyticsRequest;
    ...
    if (!isAdmin) {
      ...
      if (!partnerRole) {
        return new Response(
          JSON.stringify({ error: "Only partners can access analytics" }),
          {
            status: 403,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          }
    ...
    const redeemedSales = vouchers?.filter(v => v.status === 'redeemed')
      .reduce((sum, v) => sum + Number(v.voucher_amount), 0) || 0;

    // Sales trend (daily aggregation)
    const salesByDay: Record<string, { sales: number; count: number; commission: number }> = {};
    
    vouchers?.forEach(voucher => {
      const date = new Date(voucher.created_at).toISOString().split('T')[0];
      if (!salesByDay[date]) {
        salesByDay[date] = { sales: 0, count: 0, commission: 0 };
      }
    ...
    // Top selling amounts
    const voucherAmounts: Record<number, number> = {};
    vouchers?.forEach(voucher => {
      const amount = Number(voucher.voucher_amount);
      voucherAmounts[amount] = (voucherAmounts[amount] || 0) + 1;
    });

    const topSellingAmounts = Object.entries(voucherAmounts)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 10)
      .map(([amount, count]) => ({
    ...
    vouchers?.filter(v => v.status === 'redeemed').forEach(voucher => {
      const date = voucher.redeemed_at 
        ? new Date(voucher.redeemed_at).toISOString().split('T')[0]
        : new Date(voucher.created_at).toISOString().split('T')[0];
      
      commissionByDay[date] = (commissionByDay[date] || 0) + Number(voucher.commission_amount);
    });

    const commissionTrend = Object.entries(commissionByDay)
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([date, commission]) => ({
        date,
        commission,
    ...
    let partnerPerformance: any[] = [];
    
    if (isAdmin && !partner_id) {
      const { data: allVouchers } = await supabaseClient
        .from('vouchers')
        .select(`
          *,
          partner:partner_id (
      ...
      allVouchers?.forEach(voucher => {
        ...
        if (!partnerStats[pid]) {
          partnerStats[pid] = {
            partner_id: pid,
            partner_name: (voucher as any).partner?.username || 'Unknown',
            total_vouchers: 0,
            redeemed_vouchers: 0,
            total_sales: 0,
            total_commission: 0,
      ...
      partnerPerformance = Object.values(partnerStats)
