...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: configData, error: configError } = await supabase.from('platform_config').select('value').eq('key', 'email_verification_reminders').single();
    if (configError) {
    ...
    const { data: unverifiedUsers, error: usersError } = await supabase.from('profiles').select('id, username, email, created_at').eq('email_verified', false).not('email', 'is', null);
    if (usersError) {
    ...
    for (const user of unverifiedUsers || []){
      try {
        ...
        const { data: reminderHistory, error: historyError } = await supabase.from('email_verification_reminders').select('*').eq('user_id', user.id).single();
        ...
        if (!reminderHistory) {
          ...
          const daysSinceLastReminder = reminderHistory.last_reminder_sent_at ? Math.floor((now.getTime() - new Date(reminderHistory.last_reminder_sent_at).getTime()) / (1000 * 60 * 60 * 24)) : 999;
        ...
        if (shouldSendReminder) {
          ...
          if (!reminderHistory) {
            ...
            await supabase.from('email_verification_reminders').update({
