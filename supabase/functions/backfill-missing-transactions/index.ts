...
interface BackfillResult {
  taskCompletionTransactionsCreated: number;
  referralEarningTransactionsCreated: number;
  errors: any[];
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: taskCompletions, error: tcError } = await supabase
      ...
      .select('id, user_id, task_id, earnings_amount, completed_at, is_correct')
      .eq('is_correct', true)
      .gt('earnings_amount', 0)
      .order('completed_at', { ascending: true });

    if (tcError) {
      console.error('Error fetching task completions:', tcError);
      result.errors.push({ type: 'task_completions_fetch', error: tcError });
      ...
      for (const tc of taskCompletions) {
        try {
          ...
          const { data: existingTx } = await supabase
            ...
            .contains('metadata', { task_id: tc.task_id })
            .maybeSingle();
          ...
          const { data: profile } = await supabase
            .from('profiles')
            .select('earnings_wallet_balance, username')
            .eq('id', tc.user_id)
            .single();

          if (!profile) {
            console.error(`  ❌ Profile not found for user ${tc.user_id}`);
            result.errors.push({ type: 'profile_not_found', user_id: tc.user_id, task_completion_id: tc.id });
            continue;
          }
          ...
          if (insertError) {
            ...
            result.errors.push({ type: 'transaction_insert', task_completion_id: tc.id, error: insertError });
          } else {
          ...
          result.errors.push({ type: 'processing_error', task_completion_id: tc.id, error: error.message });
        }
      }
    }
    ...
    const { data: referralEarnings, error: reError } = await supabase
      .from('referral_earnings')
      .select('id, referrer_id, referred_user_id, earning_type, commission_amount, created_at, metadata')
      .order('created_at', { ascending: true });

    if (reError) {
      console.error('Error fetching referral earnings:', reError);
      result.errors.push({ type: 'referral_earnings_fetch', error: reError });
      ...
      for (const re of referralEarnings) {
        try {
          ...
          const { data: referrer } = await supabase
            .from('profiles')
            .select('earnings_wallet_balance, username')
            .eq('id', re.referrer_id)
            .single();

          if (!referrer) {
            console.error(`  ❌ Referrer profile not found for ${re.referrer_id}`);
            result.errors.push({ type: 'referrer_not_found', referrer_id: re.referrer_id, referral_earning_id: re.id });
            continue;
          }
          ...
          if (insertError) {
            ...
            result.errors.push({ type: 'transaction_insert', referral_earning_id: re.id, error: insertError });
          } else {
          ...
          result.errors.push({ type: 'processing_error', referral_earning_id: re.id, error: error.message });
        }
      }
    }
