...
const handler = async (req)=>{
  ...
  try {
    ...
    const { data: emailLog, error: logError } = await supabase.from("email_logs").select("metadata").eq("id", emailLogId).single();
    ...
    const resendResponse = await fetch(`https://api.resend.com/emails/${resendId}`, {
      headers: {
