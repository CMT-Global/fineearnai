...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!tiers || tiers.length === 0) {
      ...
      return new Response(JSON.stringify({
        success: false,
        message: 'No active bonus tiers configured'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    const { data: partners, error: partnersError } = await supabase.from('partner_config').select('user_id').eq('is_active', true);
    if (partnersError) {
    ...
    if (!partners || partners.length === 0) {
      ...
      return new Response(JSON.stringify({
        success: true,
        message: 'No active partners to process',
    ...
    const { data: salesData, error: salesError } = await supabase.from('vouchers').select('partner_id, voucher_amount').eq('status', 'redeemed').gte('redeemed_at', lastSunday.toISOString()).lte('redeemed_at', lastSaturday.toISOString());
    if (salesError) {
      console.error('[CALCULATE-BONUSES] Error fetching sales:', salesError);
      throw salesError;
    }
    // Group sales by partner
    const partnerSalesMap = new Map();
    if (salesData) {
    ...
    for (const partner of partners){
      ...
      const { error: upsertError } = await supabase.from('partner_weekly_bonuses').upsert({
