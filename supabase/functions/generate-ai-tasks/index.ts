...
11. Stay within the word limits: Prompt max ${template?.maxPromptWords[difficulty] || 'specified'} words, Responses max ${template?.maxResponseWords[difficulty] || 'specified'} words each`;
    ...
    while(remainingToGenerate > 0 && retryCount <= maxRetries){
      ...
      const { data: existingTasks, error: checkError } = await supabase.from('ai_tasks').select('prompt').in('prompt', promptsToCheck);
      if (checkError) {
      ...
      if (uniqueTasks.length > 0) {
        ...
        const { data: recentTasks } = await supabase.from('ai_tasks').select('prompt').eq('category', category).order('created_at', {
          ascending: false
      ...
      if (uniqueTasks.length > 0) {
        const { data: insertedTasks, error: insertError } = await supabase.from('ai_tasks').insert(uniqueTasks).select();
        if (insertError) {
      ...
      if (remainingToGenerate > 0 && retryCount < maxRetries) {
        ...
        const { data: allExistingTasks } = await supabase.from('ai_tasks').select('prompt').eq('category', category);
