...
serve(async (req)=>{
  ...
  try {
    ...
    // Verify admin role
    const { data: roleData, error: roleError } = await supabase.from('user_roles').select('role').eq('user_id', user.id).eq('role', 'admin').single();
...
async function handleMarkPaidManually(supabase, withdrawal, adminId, notes) {
  ...
  const { error: txnError } = await supabase.from('transactions').update({
    status: 'completed'
  }).eq('user_id', withdrawal.user_id).eq('type', 'withdrawal').eq('amount', withdrawal.amount).eq('status', 'pending').order('created_at', {
    ascending: false
  }).limit(1);
...
// Action: Pay Via API
async function handlePayViaAPI(supabase, withdrawal, adminId) {
  ...
  try {
    ...
    if (apiResponse.status === 'processing' && apiResponse.cpay_withdrawal_id) {
      ...
      await supabase.from('withdrawal_requests').update({
        ...
        processed_by: adminId,
        processed_at: new Date().toISOString(),
        manual_txn_hash: apiResponse.cpay_withdrawal_id,
...
// Action: Reject
async function handleReject(supabase, withdrawal, adminId, rejectionReason) {
  ...
  const { data: userProfile, error: profileError } = await supabase.from('profiles').select('earnings_wallet_balance').eq('id', withdrawal.user_id).single();
  ...
  const { error: refundTxnError } = await supabase.from('transactions').insert({
    user_id: withdrawal.user_id,
    type: 'adjustment',
    amount: withdrawal.amount,
...
function detectPaymentProvider(paymentMethod) {
  const method = paymentMethod.toLowerCase();
  // Match CPAY-related payment methods
  // Includes: cpay, crypto payout, and all branded wallet options
  if (method.includes('cpay') || method.includes('crypto payout') || method.includes('gcrypto') || method.includes('binance') || method.includes('coins.ph') || method.includes('bybit') || method.includes('coinbase') || method.includes('kucoin')) {
    return 'cpay';
  }
...
async function processCPAYWithdrawal(withdrawal) {
  ...
  try {
    ...
    if (!accountAuthResponse.ok) {
      const errorMsg = accountAuthResponse.status === 401 || accountAuthResponse.status === 403 ? 'Invalid CPAY API credentials (check publicKey/privateKey in secrets)' : `CPAY account authentication failed (${accountAuthResponse.status}): ${accountAuthText}`;
    ...
    if (!walletAuthResponse.ok) {
      const errorMsg = walletAuthResponse.status === 401 ? 'Invalid wallet credentials (check walletId/passphrase in secrets)' : walletAuthResponse.status === 403 ? 'Wallet access forbidden (check wallet permissions in CPAY dashboard)' : `CPAY wallet authentication failed (${walletAuthResponse.status}): ${walletAuthText}`;
    ...
    const withdrawalPayload = {
      ...
      amount: formatAmount(parseFloat(withdrawal.net_amount), 2),
...
async function processPayeerWithdrawal(withdrawal) {
