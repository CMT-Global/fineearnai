...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: partnerConfig, error: partnerError } = await supabase.from('partner_config').select('is_active, current_rank').eq('user_id', user.id).single();
    if (partnerError || !partnerConfig || !partnerConfig.is_active) {
      return new Response(JSON.stringify({
        error: 'Not an active partner'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    const { data: salesData, error: salesError } = await supabase.from('vouchers').select('voucher_amount').eq('partner_id', user.id).eq('status', 'redeemed').gte('redeemed_at', currentSunday.toISOString()).lte('redeemed_at', now.toISOString());
    if (salesError) {
    ...
    const { data: tiers, error: tiersError } = await supabase.from('partner_bonus_tiers').select('*').eq('is_active', true).order('tier_order', {
      ascending: true
    });
    if (tiersError) {
    ...
    const progressToNextTier = nextTier ? (currentWeekSales - (currentTier?.min_weekly_sales || 0)) / (nextTier.min_weekly_sales - (currentTier?.min_weekly_sales || 0)) * 100 : 100;
    const amountToNextTier = nextTier ? Math.max(0, nextTier.min_weekly_sales - currentWeekSales) : 0;
