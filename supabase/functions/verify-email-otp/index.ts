...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: otpRecords, error: findError } = await supabaseClient.from("email_verification_otps").select("*").eq("user_id", userId).eq("otp_code", otpCode).is("used_at", null).order("created_at", {
      ascending: false
    }).limit(1);
    if (findError) {
    ...
    const { error: updateOTPError } = await supabaseClient.from("email_verification_otps").update({
    ...
    const { error: cleanupError } = await supabaseClient.from("email_verification_otps").delete().eq("user_id", userId).neq("id", otpRecord.id);
    if (cleanupError) {
