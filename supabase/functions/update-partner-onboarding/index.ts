...
serve(async (req)=>{
  ...
  try {
    ...
    console.log('[Partner Onboarding] Update request:', {
      user_id: user.id,
      step,
      value,
      dismiss
    ...
    const { data: existingOnboarding, error: fetchError } = await supabase.from('partner_onboarding').select('*').eq('partner_id', user.id).maybeSingle();
    if (fetchError) {
    ...
    if (!onboarding) {
      ...
      const { data: created, error: createError } = await supabase.from('partner_onboarding').insert({
        partner_id: user.id,
    ...
    // Update the record
    const { data: updated, error: updateError } = await supabase.from('partner_onboarding').update(updateData).eq('partner_id', user.id).select().single();
    if (updateError) {
      throw updateError;
    }
    return new Response(JSON.stringify({
