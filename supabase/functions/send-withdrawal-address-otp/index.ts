...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { usdcAddress, usdtAddress }: OTPRequest = await req.json();

    // Validate at least one address is provided
    if (!usdcAddress && !usdtAddress) {
      return new Response(JSON.stringify({ error: 'At least one cryptocurrency address is required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (recentOTP && recentOTP.length > 0) {
      return new Response(JSON.stringify({ error: 'Please wait at least 1 minute before requesting another OTP' }), {
        status: 429,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
