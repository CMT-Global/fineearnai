...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // Fetch application
    const { data: application, error: fetchError } = await supabaseClient.from("partner_applications").select("*").eq("id", body.application_id).single();
    ...
    const { data: applicantProfile } = await supabaseClient.from("profiles").select("username, email, full_name").eq("id", application.user_id).single();
    if (body.action === "approve") {
      console.log("[Admin Partner] Approving application...");
      // Step 1: Update application status
      const { error: updateAppError } = await supabaseClient.from("partner_applications").update({
        status: "approved",
        reviewed_by: user.id,
      ...
      const { error: configError } = await supabaseClient.from("partner_config").insert({
        user_id: application.user_id,
        commission_rate,
      ...
      const { error: onboardingError } = await supabaseClient.from("partner_onboarding").insert({
