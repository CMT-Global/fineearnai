...
serve(async (req)=>{
  ...
  try {
    ...
    // Find the OTP record
    const { data: otpRecord, error: otpError } = await supabaseAdmin.from('account_deletion_otps').select('*').eq('user_id', userId).eq('otp_code', otp_code).is('used_at', null).order('created_at', {
      ascending: false
    }).limit(1).single();
    ...
    // Mark OTP as used
    const { error: updateError } = await supabaseAdmin.from('account_deletion_otps').update({
    ...
    const { data: profile } = await supabaseAdmin.from('profiles').select('username, email').eq('id', userId).single();
    // Log the deletion event in audit logs BEFORE deletion
    const { error: auditError } = await supabaseAdmin.from('user_activity_log').insert({
      user_id: userId,
