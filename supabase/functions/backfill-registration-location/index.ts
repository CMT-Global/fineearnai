...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: usersToFix, error: queryError } = await supabase
      .from('profiles')
      .select('id, username, email, last_login_country, last_login_country_name, last_login_ip, created_at')
      .is('registration_country', null)
      .not('last_login_country', 'is', null);
