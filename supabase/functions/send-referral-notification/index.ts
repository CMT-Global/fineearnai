...
serve(async (req)=>{
  ...
  try {
    ...
    // Parse request body
    const { referrer_id, referred_id, referred_username, referred_email, referral_code } = await req.json();
    ...
    const { data: referrerProfile, error: referrerError } = await supabase.from('profiles').select(`
        id,
        username,
        email,
        membership_plan,
    ...
    const { data: referrerPlan, error: planError } = await supabase.from('membership_plans').select('task_commission_rate, deposit_commission_rate').eq('name', referrerProfile.membership_plan).single();
    ...
    const { data: referralStats, error: statsError } = await supabase.from('referrals').select('total_commission_earned').eq('referrer_id', referrer_id);
    const totalReferrals = referralStats?.length || 0;
    const totalCommission = referralStats?.reduce((sum, ref)=>sum + (parseFloat(ref.total_commission_earned) || 0), 0) || 0;
    ...
    if (REFERRAL_MILESTONES.includes(totalReferrals)) {
      ...
      const { data: existingMilestone } = await supabase.from('email_logs').select('id').eq('recipient_user_id', referrer_id).eq('subject', 'Referral Milestone Achieved!').ilike('body', `%${totalReferrals}%`).limit(1).maybeSingle();
