 ...
 */ Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: users, error: queryError } = await supabase.from('profiles').select('id, username, registration_country, registration_country_name, preferred_currency').eq('preferred_currency', 'USD').not('registration_country', 'is', null);
    if (queryError) {
