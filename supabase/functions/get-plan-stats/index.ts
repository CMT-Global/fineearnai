...
const handler = async (req)=>{
  ...
  try {
    ...
    const { data: userCounts, error: countError } = await supabase.from('profiles').select('membership_plan');
    if (countError) {
    ...
    const { data: plans, error: plansError } = await supabase.from('membership_plans').select('name, display_name, account_type').eq('is_active', true).order('account_type', {
      ascending: true
    }).order('price', {
      ascending: true
    });
    ...
    return new Response(JSON.stringify(planStats), {
      status: 200,
      headers: {
        ...corsHeaders,
        'Content-Type': 'application/json'
