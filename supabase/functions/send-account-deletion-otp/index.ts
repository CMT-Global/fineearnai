...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabaseClient.from('profiles').select('email, username').eq('id', userId).single();
    ...
    const { data: recentOTPs, error: recentError } = await supabaseClient.from('account_deletion_otps').select('id').eq('user_id', userId).gte('created_at', fifteenMinutesAgo);
    if (recentError) {
    ...
    const { error: invalidateError } = await supabaseClient.from('account_deletion_otps').update({
    ...
    const { data: otpRecord, error: otpError } = await supabaseClient.from('account_deletion_otps').insert({
      user_id: userId,
      email: userEmail,
      otp_code: otpCode,
    ...
    const { data: emailResponse, error: emailError } = await supabaseClient.functions.invoke('send-template-email', {
      body: {
        email: userEmail,
