...
serve(async (req)=>{
  ...
  try {
    ...
    const { amount, currency = 'USDT', processorId } = await req.json();
    if (!amount || amount <= 0) {
      throw new Error('Invalid amount');
    }
    console.log(`[CPAY-DEPOSIT] User: ${user.id}, Amount: ${amount} ${currency}, ProcessorId: ${processorId || 'auto'}`);
    // Get user profile
    const { data: profile, error: profileError } = await supabase.from('profiles').select('email, username, deposit_wallet_balance').eq('id', user.id).single();
    if (profileError) {
    ...
    if (processorId) {
      ...
      const { data: processor, error: procError } = await supabase.from('payment_processors').select('*').eq('id', processorId).single();
      if (procError) {
      ...
      // FALL BACK to auto-select instead of erroring
        ...
        const { data: boundCheckout, error: boundError } = await supabase.from('cpay_checkouts').select('*').eq('id', checkoutId).single();
    ...
    if (!checkout) {
      ...
      const { data: autoCheckout, error: checkoutError } = await supabase.from('cpay_checkouts').select('*').eq('currency', currency).eq('is_active', true).gte('max_amount', amount).lte('min_amount', amount).limit(1).single();
      if (checkoutError || !autoCheckout) {
        console.error('[CPAY-DEPOSIT] No active checkout found for currency:', currency, checkoutError);
        throw new Error(`No active CPAY checkout matches currency ${currency} and amount $${amount}. ` + `Please configure a checkout in Admin > CPAY Checkouts, or update processor binding.`);
      }
    ...
    // Build CPAY checkout URL with clickId parameter (CPAY's tracking mechanism)
    const checkoutUrl = `https://checkouts.cpay.world/checkout/${checkout.checkout_id}?clickId=${orderId}`;
    console.log(`[CPAY-DEPOSIT] 🔗 Checkout URL: ${checkoutUrl}`);
    ...
    console.log(`[CPAY-DEPOSIT] ✅ SUCCESS: Transaction ${transaction.id}, Order ${orderId}`);
