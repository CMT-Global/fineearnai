...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: stuckJobs, error: fetchError } = await supabase.from("bulk_email_jobs").select("id, batch_id, processing_worker_id, last_heartbeat, processed_count, total_recipients, processing_metadata, retry_count, max_retries").eq("status", "processing").not("last_heartbeat", "is", null).lt("last_heartbeat", tenMinutesAgo);
    if (fetchError) {
    ...
    for (const job of stuckJobs){
      ...
      if (retryCount >= maxRetries) {
        ...
        const { error: failError } = await supabase.from("bulk_email_jobs").update({
          status: "failed",
      ...
      const { error: resetError } = await supabase.from("bulk_email_jobs").update({
        status: "queued",
