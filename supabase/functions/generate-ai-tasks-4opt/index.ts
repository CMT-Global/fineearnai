...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Check admin or task_manager_4opt role
    const { data: roleData } = await supabase.from('user_roles').select('role').eq('user_id', user.id).in('role', ['admin', 'task_manager_4opt']).limit(1).maybeSingle();
    if (!roleData) {
      throw new Error('Admin or task_manager_4opt access required');
    }
...
FOUR-OPTION REQUIREMENTS (CRITICAL):
8. Each task MUST have EXACTLY 4 options (A, B, C, D)
9. All 4 options must be plausible and clearly distinct - avoid obvious wrong answers
10. correct_response must be randomly distributed across a, b, c, d - NO "Option A always correct" pattern
11. Avoid repeating options or having two options with the same meaning

Return ONLY a valid JSON array (no markdown, no explanation) with this exact structure:
[
  {
    "prompt": "The task prompt/question",
    "response_a": "Option A text",
    "response_b": "Option B text",
    "response_c": "Option C text",
    "response_d": "Option D text",
    "correct_response": "a" or "b" or "c" or "d",
...
- Maximum Prompt Words: ${template.maxPromptWords[difficulty]} words
- Maximum Response Words: ${template.maxResponseWords[difficulty]} words per option
- Recommended Vocabulary: ${template.vocabulary.join(', ')}

EXAMPLE FOR ${difficulty.toUpperCase()} LEVEL:
${template.examples[difficulty]}
` : ''}
...
2. Provide EXACTLY 4 distinct options (A, B, C, D). Extend template format to 4 options: ${template?.responseFormat || 'Make 4 options distinct'}
3. ${category.includes('Sentiment') || category.includes('Review') || category.includes('Feedback') ? 'For sentiment: include options like Positive, Negative, Neutral, Mixed - make 4 plausible distinct choices' : 'Make all 4 options clearly distinct and easy to understand'}
...
5. Avoid passive voice (say "The staff helped me" not "I was helped by the staff")
    ...
    10. All 4 response options must be clearly written and easy to understand
    ...
    // Prepare initial tasks for insertion (validate 4 options)
    let tasksToInsert = tasks.filter(t => t.response_a && t.response_b && t.response_c && t.response_d && ['a','b','c','d'].includes(t.correct_response)).map(task => ({
      prompt: task.prompt,
      response_a: task.response_a,
      response_b: task.response_b,
      response_c: task.response_c,
      response_d: task.response_d,
    ...
    while(remainingToGenerate > 0 && retryCount <= maxRetries){
      ...
      const { data: existingTasks, error: checkError } = await supabase.from('ai_tasks_4opt').select('prompt').in('prompt', promptsToCheck);
      if (checkError) {
      ...
      if (uniqueTasks.length > 0) {
        ...
        const { data: recentTasks } = await supabase.from('ai_tasks_4opt').select('prompt').eq('category', category).order('created_at', {
          ascending: false
        }).limit(50); // Check against 50 most recent tasks in category
        if (recentTasks && recentTasks.length > 0) {
          const semanticallyUniqueTasks = [];
      ...
      if (uniqueTasks.length > 0) {
        const { data: insertedTasks, error: insertError } = await supabase.from('ai_tasks_4opt').insert(uniqueTasks).select();
        if (insertError) {
      ...
      if (remainingToGenerate > 0 && retryCount < maxRetries) {
        ...
        const { data: allExistingTasks } = await supabase.from('ai_tasks_4opt').select('prompt').eq('category', category);
...
- Max Prompt Words: ${template.maxPromptWords[difficulty]}
- Max Response Words: ${template.maxResponseWords[difficulty]}
` : ''}
...
- Make all 4 response options clear and easy to understand
...
5. Follow all language simplicity and template guidelines`;
        ...
        try {
          ...
          if (Array.isArray(retryTasks) && retryTasks.length > 0) {
            tasksToInsert = retryTasks.filter((t: Task) => t.response_a && t.response_b && t.response_c && t.response_d && ['a','b','c','d'].includes(t.correct_response)).map((task: Task) => ({
              prompt: task.prompt,
              response_a: task.response_a,
              response_b: task.response_b,
              response_c: task.response_c,
    ...
    // Final response
    const finalMessage = remainingToGenerate > 0 ? `Created ${allInsertedTasks.length}/${quantity} tasks. Skipped ${totalDuplicatesSkipped} exact duplicate(s) and ${totalSemanticDuplicatesSkipped} semantic duplicate(s). Could not generate ${remainingToGenerate} more unique task(s) after ${retryCount} retries.` : totalDuplicatesSkipped > 0 || totalSemanticDuplicatesSkipped > 0 ? `Created ${allInsertedTasks.length} tasks. Skipped ${totalDuplicatesSkipped} exact duplicate(s) and ${totalSemanticDuplicatesSkipped} semantic duplicate(s) with ${retryCount} retry attempts.` : `Created ${allInsertedTasks.length} tasks successfully.`;
    ...
    console.error('Error in generate-ai-tasks-4opt:', error);
    return new Response(JSON.stringify({
