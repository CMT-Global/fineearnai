...
const VALID_STEPS = [1, 2, 3, 4, 5, 6, 7] as const;
...
serve(async (req) => {
  ...
  try {
    ...
    const step = typeof body?.step === 'number' && VALID_STEPS.includes(body.step as typeof VALID_STEPS[number]) ? body.step : 1;
    const toEmail = (body?.test_email && typeof body.test_email === 'string' && body.test_email.includes('@')) ? body.test_email : user.email;
    ...
    const { data, error } = await supabaseAdmin.functions.invoke('send-template-email', {
      body: {
        email: toEmail,
        template_type: `trial_reactivation_${step}`,
        variables: {
          first_name: 'there',
          login_url: loginUrl,
